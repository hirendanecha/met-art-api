...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in Metart Tube channel by the Metart Tube Admin.
  To access your channel, log into your Metart Tube account,click on the
  Metart Tube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `Metart Tube has approved your Connection application.`;
    } else {
      msg = `Metart Tube has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
